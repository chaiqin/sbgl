<?php /* Smarty version Smarty-3.1.6, created on 2018-01-30 15:38:07
         compiled from "Admin/View\User\userList.html" */ ?>
<?php /*%%SmartyHeaderCode:89155a7020df936184-00408321%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  'file_dependency' => 
  array (
    '096c28ba00b161ab22645bc16eb6885bf2849bdb' => 
    array (
      0 => 'Admin/View\\User\\userList.html',
      1 => 1505999395,
      2 => 'file',
    ),
  ),
  'nocache_hash' => '89155a7020df936184-00408321',
  'function' => 
  array (
  ),
  'has_nocache_code' => false,
  'version' => 'Smarty-3.1.6',
  'unifunc' => 'content_5a7020dfbb85d',
),false); /*/%%SmartyHeaderCode%%*/?>
<?php if ($_valid && !is_callable('content_5a7020dfbb85d')) {function content_5a7020dfbb85d($_smarty_tpl) {?><!DOCTYPE html>
<html>
    <head>
        <meta charset="utf-8">
        <title>
...
/Admin/css/extra.css" media="all">
    ...
    <body>
        ...
        <div class="x-body">
            ...
            <xblock><button class="layui-btn layui-btn-danger" onclick="delAll()"><i class="layui-icon">&#xe640;</i>批量删除</button><button class="layui-btn" onclick="add('添加用户', '<?php echo @__CONTROLLER__;?>
/addUser', '700', '500')"><i class="layui-icon">&#xe608;</i>添加</button><span class="x-right" style="line-height:40px">共有数据：<span id="total"></span> 条</span></xblock>
...
/userList",
                        ...
                        for (var i = 0; i < list.length; i++) {
                            ...
                            str += "	<a title='编辑' href='javascript:;' onclick=" + '"' + "edit('编辑','<?php echo @__CONTROLLER__;?>
/reviseUser/id/" + list[i]['id'] + "','4','700','500')" + '"' +
...
/deleteUser", {id: s}, function (data) {
                ...
                function member_stop(obj, id) {
                    layer.confirm('确认要停用吗？', function (index) {
                        //发异步把用户状态进行更改
                        $.post("<?php echo @__CONTROLLER__;?>
/changeStatus", {id: id, status: 0}, function (data) {
                            if (data.msg) {
                                layer.msg(data.msg, {icon: 1, time: 1000});
                                if (data.status) {
                ...
                function member_start(obj, id) {
                    layer.confirm('确认要启用吗？', function (index) {
                        //发异步把用户状态进行更改
                        $.post("<?php echo @__CONTROLLER__;?>
/changeStatus", {id: id, status: 1}, function (data) {
                            if (data.msg) {
                                layer.msg(data.msg, {icon: 1, time: 1000});
                                if (data.status) {
                ...
                function del(obj, id) {
                    layer.confirm("确定删除吗？", function (index) {
                        //发异步删除数据
                        $.post("<?php echo @__CONTROLLER__;?>
/deleteUser", {id: id}, function (data) {
                            if (data.msg) {
                                layer.msg(data.msg, {icon: 1, time: 1000});
                                if (data.status) {
                                    $('input[name="uid"]:checked').each(function () {
